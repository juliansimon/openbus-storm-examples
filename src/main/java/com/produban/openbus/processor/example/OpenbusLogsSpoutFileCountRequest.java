...
import com.produban.openbus.processor.function.LogRequestCountByTime;
...
public class OpenbusLogsSpoutFileCountRequest {	
	private static Logger LOG = LoggerFactory.getLogger(OpenbusLogsSpoutFileCountRequest.class);
	...
	public static StormTopology buildTopology() {	
		...
		Stream termStream = topology.newStream("spout", spout)				
				.each(new Fields("rawLogs"), new LogRequestCountByTime(), new Fields("request"));
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 0) {		
		    ...
		    cluster.submitTopology("fileCountRequest", conf, buildTopology());	
		    Thread.sleep(1000);
