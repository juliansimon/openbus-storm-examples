 ...
 */
public class AvroDecoderLogs extends BaseFunction {

	Logger LOG = LoggerFactory.getLogger(AvroDecoderLogs.class);
	private static final long serialVersionUID = 3612507014289114967L;
	...
	public final void execute(final TridentTuple tuple, final TridentCollector collector) {
		...
		try {									
		    ...
		    if (LOG.isDebugEnabled())  LOG.debug("Message Avro: " + result.toString());
		    		   
		    collector.emit(new Values(result.get("host"), result.get("log"), result.get("user"), result.get("datetime"), 
		    		result.get("request"), result.get("status"), result.get("size"), result.get("referer"), 
		    		result.get("userAgent"), result.get("session"), result.get("responseTime")));
