 ...
 */
public class WebServerLog2JsonTS extends BaseFunction {
	
	private static Logger LOG = LoggerFactory.getLogger(WebServerLog2JsonTS.class);
    ...
    public final void execute(final TridentTuple tuple, final TridentCollector collector) {        
        try {        	        	
			...
			long timestamp = FormatUtil.getDateInFormatTimeStamp(json.getString("datetime"));		    
            collector.emit(new Values(json.get("host"), 
            		json.get("log"), 
            		json.get("user"), 
            		json.get("datetime"), 
		    		json.get("request"), 
		    		...
		    		json.get("userAgent"), 
		    		json.get("session"), 
		    		json.get("responseTime"), 		    		
		    		String.valueOf(timestamp),
		    		json.toString(),
		    		json.toString().substring(0, json.toString().length() -1) 
