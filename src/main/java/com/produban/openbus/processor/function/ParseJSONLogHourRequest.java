 ...
 */
public class ParseJSONLogHourRequest extends BaseFunction {

	private static Logger LOG = LoggerFactory.getLogger(ParseJSONLogHourRequest.class);    
    ...
    public final void execute(final TridentTuple tuple, final TridentCollector collector) {        
        try {
            ...
            collector.emit(new Values(request + "__" + datetime));               
        } catch (Exception e) {
