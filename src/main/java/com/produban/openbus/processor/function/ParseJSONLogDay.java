...
/**
 * Function Storm/Trident for parse Json    
 */
public class ParseJSONLogDay extends BaseFunction {

	private static Logger LOG = LoggerFactory.getLogger(ParseJSONLogDay.class);    
    ...
    public final void execute(final TridentTuple tuple, final TridentCollector collector) {        
        try {
            ...
            String datetime = FormatUtil.getDateInFormatDay(json.getString("datetime"));
            String request = FormatUtil.getRequestFormat(json.getString("request"));
            
            collector.emit(new Values(json.getString("user"), request, datetime));               
        } catch (Exception e) {
