 ...
 */
public class DateTimeInMonth extends BaseFunction {	
	private static final long serialVersionUID = 1605431104238472394L;
	private static Logger LOG = LoggerFactory.getLogger(DateTimeInMonth.class);    
    ...
    public final void execute(final TridentTuple tuple, final TridentCollector collector) {        
        try {
        	String datetime = FormatUtil.getDateFormat(tuple.getString(1), 
        			FormatUtil.DATE_FORMAT_DAY, FormatUtil.DATE_FORMAT_MONTH);
                        
            if (LOG.isDebugEnabled())  LOG.debug("Datetime in month: " + datetime);
                        
            collector.emit(new Values(tuple.getString(0) + OpenbusConstant.SEPARATOR_FIELDS + datetime, datetime));            	        	            
        } catch (Exception e) {
        	LOG.error("Format timestamp month: " + tuple.getString(0) + " " + tuple.getString(1), e); 
		} 
    }
}